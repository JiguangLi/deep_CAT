...
def train_double_dqn(
    ...
    """Training double Deep adaptive learning models
    ...
    model = bcat.QNetwork(
        ...
        num_items=m,
        hidden_dim_phi1=256,
        hidden_dim_stats=input_stats_dim
    ).to(device)
    target_network = bcat.QNetwork(
        input_dim=input_dim,
        input_stats_dim=input_stats_dim,
        num_items=m,
        hidden_dim_phi1=256,
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.5, mode='min',
                                                           patience=lr_decrease_patience,
                                                           verbose=True, min_lr=1e-8)
    early_stopping = EarlyStopping(early_stopping)
    buffer = bcat.ReplayBuffer(buffer_size)
    ...
    for episode in range(int(num_episodes)):
        ...
        if factor_generate_params:
            theta = np.random.normal(0, np.where(np.random.rand() <= factor_generate_params["q"], factor_generate_params["std"], 1), k)
        else:
            theta = np.random.normal(0, 1, k)
        episode_learner = bcat.EpisodeLearner(
            point_theta=theta,
            num_items=m,
        ...
        while True:
            ...
            if buffer.size() > batch_size:
                ...
                pos_param, pos_stats, pos_masks = zip(*sampled_states)
                ...
                next_pos_param, next_pos_stats,  next_pos_masks = zip(*sampled_next_states)
                ...
                with torch.no_grad():
                    next_actions = model(next_pos_params, next_pos_stats, next_pos_masks).argmax(
                        1, keepdim=True)  # CHANGED: eval_network selects actions
                    next_q_values = target_network(next_pos_params, next_pos_stats,
                                                   next_pos_masks).gather(1, next_actions)
                    target_q_values = sampled_rewards + (1 - dones) * gamma * next_q_values

                # Compute the current Q-values
                q_values = model(pos_params, pos_stats ,pos_masks).gather(1, sampled_actions)
                if episode % 200 ==0:
                    print("q_values")
        ...
        detailed_log[episode] = {"learned_reward": learner_reward, "theta": theta, "learned_loss": learned_loss, "lr": optimizer.param_groups[0]["lr"] , "epsilon": cur_epsilon}
...
def train_dqn(
    ...
    """Training Deep adaptive learning models
    ...
    if retrain_params:
        retrain_version = retrain_params["retrain_version"]
        log_name = f"retrain_{retrain_version}_global_training_log.txt"
        log_file_global = pathlib.Path(retrain_params["network_dir"]).joinpath(log_name)
    else:
        log_file_global = pathlib.Path(save_path).joinpath("global_training_log.txt")
    log_global = open(log_file_global, "w")
    detailed_log = {}
    ...
    for episode in range(int(num_episodes)):
            ...
            if buffer.size() > batch_size:
                ...
                with torch.no_grad():
                    target_q_values = sampled_rewards + (1 - dones) * gamma * target_network(
                        next_pos_params, next_pos_stats, next_pos_masks).max(1, keepdim=True)[0]
                # Compute the current Q-values
                q_values = model(pos_params, pos_stats, pos_masks).gather(1, sampled_actions)
                # Compute the loss
                loss = criterion(q_values, target_q_values)
        ...
        if episode % save_frequency == 0:
            if retrain_params:
                network_folder_dir = retrain_params["network_folder_dir"]
                retrain_version = retrain_params["retrain_version"]
                retrain_model_name = f"{network_folder_dir}/retrain_{retrain_version}_fn_{episode}.pt"
                torch.save(model.state_dict(), retrain_model_name)
                with open(f"{network_folder_dir}/retrain_{retrain_version}_detailed_log.pickle", "wb") as handle:
                    pickle.dump(detailed_log, handle, protocol=pickle.HIGHEST_PROTOCOL)
            else:
                torch.save(model.state_dict(), f"{save_path}/fn_{episode}.pt")
                with open(f"{save_path}/detailed_log.pickle", "wb") as handle:
    ...
    if retrain_params:
        network_folder_dir = retrain_params["network_folder_dir"]
        retrain_version = retrain_params["retrain_version"]
        retrain_model_name = f"{network_folder_dir}/retrain_{retrain_version}_fn_final.pt"
        torch.save(model.state_dict(), retrain_model_name)
    else:
        torch.save(model.state_dict(), f"{save_path}/fn_final.pt")
