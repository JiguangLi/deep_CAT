...
class BayesianQTestTakers:
    ...
    def __init__(
        ...
        self.d1_tensor = np.zeros((self.n , pos_num_samples, max_item, self.k))
        self.d2_array = np.zeros((self.n, pos_num_samples,max_item))
        self.s_array = np.zeros((self.n, pos_num_samples, max_item))
        self.i_count = 0
    ...
    def get_factor_samples(self, num_samples):
        ...
        """
        i_ub = self.i_count+1
        result = np.zeros(self.n, self.pos_num_samples, num_samples, self.k)
        for i in range(self.n):
            arguments = [
                (self.d1_tensor[i, j, 1:i_ub, :], self.s_array[i, j, 1:i_ub], self.d2_array[i, j, 1:i_ub], num_samples) for
                j in range(self.pos_num_samples)]
            with multiprocessing.Pool(self.num_workers) as pool:
                out = pool.starmap(self.sample_from_sun, arguments)
            result[i] = np.stack(out, axis=0)
        return result
